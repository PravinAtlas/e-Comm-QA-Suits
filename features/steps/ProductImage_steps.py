...
def capture_product_image_for_comparison(context, product_name):
    ...
    context.products_page.capture_product_image(
        product_name, context.captured_image_path
    )
...
def verify_product_image_matches_baseline(context, product_name):
    ...
    expect(os.path.exists(baseline_path), f"Baseline image not found: {baseline_path}")
    expect(
        ImageComparator.compare_images(context.captured_image_path, baseline_path),
